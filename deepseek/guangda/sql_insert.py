...
def translate_text(text, retries=3):
    for attempt in range(retries):
        try:
                ...
                messages=[
                    {
                        "role": "system",
                        "content": (
                            "你是我的助手，按我的要求完成一些重复性工作，根据create表结构，把下边168行数据，生成168行insert into 的sql;"
                        ),
                    },
            ...
            print(f"API 调用失败，重试 {attempt + 1}/{retries}：{type(e).__name__} - {str(e)}")
...
# 并发编码函数（带进度打印）
def translate_concurrently(text, max_chunk_size=50000, max_workers=10, is_english=False):
    # 动态分块
...
# 读取 sql 和 txt 文件内容
def read_sql_or_txt_file(file_path):
...
def code_word_file(input_path, output_path, is_english=False):
    ...
    try:
        ...
        elif input_path.endswith(".sql") or input_path.endswith(".txt"):
            original_text = read_sql_or_txt_file(input_path)
        else:
            print("错误：不支持的文件格式。仅支持 .docx、.pdf、.sql 和 .txt 文件。")
            return
    except Exception as e:
        print(f"读取文件失败：{e}")
        return
...
if __name__ == "__main__":
    ...
    input_file = "/Users/ly/ynworkspace/Python-100-Days/deepseek/guangda/ec.sql"  # 输入文件路径
    output_file = "/Users/ly/ynworkspace/Python-100-Days/deepseek/guangda/ec1.docx"  # 输出文件路径
